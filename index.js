...
const schemas = gql`
  ...
  type Mutation {
    ...
    updateBlog(title: String!, description: String!): Blog
    ...
    replyComment(title: String!): Blog
  }
`;
...
const blogResolvers = {
  ...
  Mutation: {
    ...
    updateBlog: (parent, args) => {
      ...
      blogs[location].description = description;
      return args;
    },
    ...
    unlikeBlog: (parent, args) => {
      ...
      if (blogs[location].likes <= 0) {
        console.log("There is nothing to unlike here");
      } else {
        blogs[location].likes -= 1;
      }
      return args;
